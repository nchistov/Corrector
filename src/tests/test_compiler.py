...
def test_if():
    ...
    assert bc == bytearray((0x00, 0x00, 0x03, 0x01, 0x0A, 0x04, 0x01, 0x0E, 0x01, 0x0D, 0x00, 0x01, 0x05, 0x0D))

def test_if_not():
    code = 'ЭТО Процедура ЕСЛИ НЕ ПУСТО ТО ВПРАВО КОНЕЦ'
    bc = c.compile(code)

    assert bc == bytearray((0x00, 0x00, 0x03, 0x01, 0x0A, 0x04, 0x01, 0x11, 0x0E, 0x01, 0x0D, 0x00, 0x01, 0x05, 0x0D))

def test_multiline():
    code = 'ЭТО Процедура ЕСЛИ ПУСТО ТО { ВПРАВО ВЛЕВО } КОНЕЦ'
    bc = c.compile(code)

    assert bc == bytearray((0x00, 0x00, 0x03, 0x01, 0x0A, 0x04, 0x01, 0x0E, 0x01, 0x0D, 0x00, 0x01, 0x05, 0x06, 0x0D))

def test_procedure_call():
    code = 'ЭТО Процедура ВПРАВО КОНЕЦ ЭТО Программа Процедура КОНЕЦ'
    bc = c.compile(code)

    assert bc == bytearray((0x00, 0x00, 0x05, 0x0D, 0x00, 0x01, 0x02, 0x00, 0x0D, 0x0D))

def test_procedure_call_before_def():
    code = 'ЭТО Программа Процедура КОНЕЦ ЭТО Процедура ВПРАВО КОНЕЦ'
    bc = c.compile(code)

    assert bc == bytearray((0x00, 0x00, 0x02, 0x01, 0x0D, 0x0D, 0x00, 0x01, 0x05, 0x0D))
